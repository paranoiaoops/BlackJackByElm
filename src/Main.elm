...
update msg model =
    case msg of
        ...
        Stand ->
            let
                newModel = dealerDrawCards model
            in
                ( { newModel
                    | status = End
                }
...
shufflePlayingCardsList cards = shuffle cards
...
calculatePoint cards =
    let
        ...
        isAce = List.member 1 cardNumbers
    in
       if isAce then
         if noAceTotal > 10 then noAceTotal + 1
         else noAceTotal + 11
       else
...
convertOverEleven number = if number >= 11 then 10 else number
...
dealerDrawCards model =
    if calculatePoint model.dealer >= 17 then model
    else
        dealerDrawCards { model
...
judgment playerCards dealerCards =
    ...
    in
        if playerPoint > 21 then "プレイヤーの負け"
        else if dealerPoint > 21 then "プレイヤーの負け"
        else if dealerPoint < playerPoint then "プレイヤーの勝ち"
        else "プレイヤーの負け"